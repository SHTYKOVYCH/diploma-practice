...
#define CBUFF_SIZE USHRT_MAX

#define RW_CHUNK_SIZE 4096
#define RW_LAST_TREE_BYTES 0xffffff
...
static ssize_t device_read(struct file* f, char __user* buff, size_t numOfBytes, loff_t* offset) {
    ...
    while (1) {
        ...
        if (readPipe->bitesAwaible) {
            ...
            if (bytesToWrite & RW_LAST_TREE_BYTES) {
            ...
            pr_info(MSG_PREFIX "sent message of %d bytes, bytes awaible %u\n", bytesToWrite, readPipe->bitesAwaible);
        ...
        if (wait_event_interruptible(readPipe->queue, readPipe->bitesAwaible) == -ERESTARTSYS) {
            return 0;
        }
    }
}
